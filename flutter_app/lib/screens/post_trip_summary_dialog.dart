...
class PostTripSummaryDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tripDuration =
        summary['trip_duration_s'] as num? ?? summary['duration'] as num? ?? 0;
    final avgAttention = summary['avg_attention'] as num? ??
        summary['attention_avg'] as num? ??
        0;
    final maxPerclos =
        summary['max_perclos'] as num? ?? summary['perclos_max'] as num? ?? 0;
              ...
              Row(children: [
                Container(
                    ...
                    child: const Icon(Icons.emoji_events_outlined,
                        color: AppColors.focusedGreen, size: 22)),
  ...
  Widget _metricsGrid(int durMin, int durSec, num avgAttention, num maxPerclos,
      num alertCount, num blinkRate) {
    return Column(children: [
      Row(children: [
        Expanded(
            child: _metricItem(Icons.timer_outlined, 'TRIP DURATION',
                '${durMin}m ${durSec}s')),
        const SizedBox(width: 12),
        Expanded(
            child: _metricItem(Icons.psychology_outlined, 'AVG ATTENTION',
      ...
      Row(children: [
        Expanded(
            child: _metricItem(
                Icons.timelapse, 'MAX PERCLOS', maxPerclos.toStringAsFixed(2))),
        const SizedBox(width: 12),
        Expanded(
            child: _metricItem(
                Icons.warning_amber_outlined, 'ALERTS', '${alertCount.toInt()}',
                isAlert: alertCount > 0)),
      ]),
      const SizedBox(height: 12),
      _metricItem(Icons.remove_red_eye_outlined, 'AVG BLINK RATE',
          '${blinkRate.toStringAsFixed(0)}/min'),
    ]);
  }
