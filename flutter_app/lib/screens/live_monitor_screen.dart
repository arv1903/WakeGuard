...
class LiveMonitorScreen extends StatefulWidget {
  const LiveMonitorScreen({super.key, required this.client, this.sessionStartTime});
...
class _LiveMonitorScreenState extends State<LiveMonitorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_fullscreenHud) {
      return _FullscreenHud(snap: snap, client: widget.client, onExit: () => setState(() => _fullscreenHud = false));
    }
          ...
          child: Column(
            ...
            children: [
              ...
              if (isNarrow)
                Column(
                  children: [
                    _VideoSection(client: widget.client, snap: snap, onFullscreen: () => setState(() => _fullscreenHud = true), sessionElapsed: _sessionElapsed),
                ...
                Row(
                  ...
                  children: [
                    Expanded(flex: 7, child: _VideoSection(client: widget.client, snap: snap, onFullscreen: () => setState(() => _fullscreenHud = true), sessionElapsed: _sessionElapsed)),
...
class _FullscreenHud extends StatelessWidget {
  const _FullscreenHud({required this.snap, required this.client, required this.onExit});
  final MonitoringSnapshot snap;
  final MonitoringClient client;
      ...
      children: [
        ...
        Positioned(top: 16, left: 16, child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 4),
            Text('${snap.fps.toStringAsFixed(0)} fps | ${snap.attention.toStringAsFixed(0)}% attention',
        ...
        Positioned(bottom: 0, left: 0, right: 0, child: Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
            _hudMetric('EAR', snap.ear != null ? snap.ear!.toStringAsFixed(2) : '--'),
            _hudMetric('PERCLOS', snap.perclos.toStringAsFixed(2)),
            _hudMetric('PITCH', '${snap.pitch.toStringAsFixed(1)}\u00B0'),
            _hudMetric('YAW', '${snap.yaw.toStringAsFixed(1)}\u00B0'),
            _hudMetric('ROLL', '${snap.roll.toStringAsFixed(1)}\u00B0'),
  ...
  Widget _hudMetric(String label, String value) {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      Text(label, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 2, color: Colors.white54)),
      const SizedBox(height: 4),
      Text(value, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, fontFamily: 'monospace', color: Colors.white)),
    ]);
  }
}
...
class _VideoSection extends StatelessWidget {
  const _VideoSection({required this.client, required this.snap, required this.onFullscreen, required this.sessionElapsed});
  final MonitoringClient client;
  final MonitoringSnapshot snap;
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Icon(Icons.fiber_manual_record, color: AppColors.focusedGreen, size: 16),
            ...
            Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                Text(sessionElapsed, style: const TextStyle(fontSize: 12, fontFamily: 'monospace', fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
              ]),
            ),
            const SizedBox(width: 8),
            // Microsleep badge
            if (snap.microsleep)
              _LiveBadge(label: 'MICROSLEEP', color: AppColors.alertRed, icon: Icons.bedtime_outlined),
            const SizedBox(width: 8),
            _ActionButton(icon: snap.alarmMuted ? Icons.volume_up : Icons.volume_off, label: snap.alarmMuted ? 'Unmute' : 'Mute',
              onTap: () => client.sendCommand(snap.alarmMuted ? '/api/v1/alarm/unmute' : '/api/v1/alarm/mute')),
            const SizedBox(width: 8),
            _ActionButton(icon: Icons.tune, label: 'Calibrate',
              onTap: () => client.sendCommand('/api/v1/calibration/start', {'duration': 4})),
            const SizedBox(width: 8),
            _ActionButton(icon: Icons.fullscreen, label: 'HUD', onTap: onFullscreen),
          ],
        ),
          ...
          child: AspectRatio(
            ...
            child: Stack(
              ...
              children: [
                ...
                Positioned(left: 12, top: 12, child: _Badge(
                  icon: snap.faceFound ? Icons.person : Icons.person_off,
                  label: snap.faceFound ? 'Driver detected' : 'No face',
                  color: snap.faceFound ? AppColors.focusedGreen : AppColors.alertOrange)),
                Positioned(right: 12, bottom: 12, child: _Badge(
                  icon: Icons.speed, label: '${snap.fps.toStringAsFixed(0)} fps', color: AppColors.textMuted)),
                Positioned(left: 12, bottom: 12, child: _Badge(
                  icon: Icons.access_time, label: 'IR-DMS', color: Colors.white54)),
              ],
            ),
          ),
        ),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(width: 4),
        Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 0.5, color: color)),
      ]),
    );
  }
}
        ...
        Card(child: Padding(
          ...
          child: Column(children: [
            ...
            ClipRRect(borderRadius: BorderRadius.circular(4), child: LinearProgressIndicator(
              value: (snap.attention / 100).clamp(0.0, 1.0), minHeight: 8,
              backgroundColor: AppColors.surface2, color: color)),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: _MiniStat(label: 'FATIGUE LVL', value: snap.emaDrowsy < 0.3 ? 'LOW' : snap.emaDrowsy < 0.6 ? 'MED' : 'HIGH',
                color: snap.emaDrowsy < 0.3 ? AppColors.focusedGreen : snap.emaDrowsy < 0.6 ? AppColors.alertAmber : AppColors.alertRed)),
              const SizedBox(width: 8),
              Expanded(child: _MiniStat(label: 'DISTRACTION', value: snap.lookingAway ? 'YES' : 'NO',
                color: snap.lookingAway ? AppColors.alertAmber : AppColors.focusedGreen)),
            ]),
          ]),
        )),
        const SizedBox(height: 16),
        ...
        Card(child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('LIVE METRICS',
              style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 2, color: AppColors.textSecondary)),
            const SizedBox(height: 16),
            _MetricWithBar(label: 'PERCLOS', value: snap.perclos.toStringAsFixed(2), pct: snap.perclos.clamp(0, 1),
              color: snap.perclos > 0.5 ? AppColors.alertRed : AppColors.focusedGreen),
            const SizedBox(height: 12),
            _MetricWithBar(label: 'Blink Rate', value: '${snap.blinksPerMin.toStringAsFixed(0)} bpm', pct: (snap.blinksPerMin / 30).clamp(0.0, 1.0),
              color: AppColors.focusedGreen),
            const SizedBox(height: 12),
            _MetricWithBar(label: 'EAR', value: snap.ear != null ? snap.ear!.toStringAsFixed(2) : '--', pct: snap.ear != null ? (snap.ear! / 0.4).clamp(0.0, 1.0) : 0,
              color: snap.ear != null && snap.ear! < 0.2 ? AppColors.alertRed : AppColors.focusedGreen),
          ]),
        )),
        const SizedBox(height: 16),
        // Head Pose
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: const [
              Text('HEAD POSE EULER ANGLES',
                style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 2, color: AppColors.textSecondary)),
              Icon(Icons.threed_rotation, size: 16, color: AppColors.textSecondary),
            ]),
            const SizedBox(height: 16),
            Row(children: [
              Expanded(child: _PoseTile(label: 'PITCH', value: '${snap.pitch.toStringAsFixed(1)}\u00B0', nominal: snap.poseValid && !snap.headDown)),
              const SizedBox(width: 8),
              Expanded(child: _PoseTile(label: 'YAW', value: '${snap.yaw.toStringAsFixed(1)}\u00B0', nominal: snap.poseValid && !snap.lookingAway)),
              const SizedBox(width: 8),
              Expanded(child: _PoseTile(label: 'ROLL', value: '${snap.roll.toStringAsFixed(1)}\u00B0', nominal: snap.poseValid && !snap.headTilt)),
            ]),
            const SizedBox(height: 12),
...
class _MetricWithBar extends StatelessWidget {
  const _MetricWithBar({required this.label, required this.value, required this.pct, required this.color});
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, fontFamily: 'monospace', color: AppColors.textPrimary)),
      ]),
      const SizedBox(height: 4),
...
class _MiniStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.surface0, borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0x33444748))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w700, letterSpacing: 1, color: AppColors.textSecondary)),
        const SizedBox(height: 4),
...
class _HeadPoseBar extends StatelessWidget {
  const _HeadPoseBar({required this.pitch, required this.yaw, required this.roll});
  ...
  Widget _barColumn(String label, double value, double max) {
    ...
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 4),
      SizedBox(width: 6, height: 24, child: Stack(children: [
        Container(decoration: BoxDecoration(color: AppColors.surface2, borderRadius: BorderRadius.circular(3))),
        Positioned(bottom: 0, child: Container(width: 6, height: 24 * pct,
...
class _AttentionGauge extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return SizedBox(width: 200, height: 110, child: CustomPaint(
      painter: _GaugePainter(value: value, color: color),
      child: Center(child: Padding(
        padding: const EdgeInsets.only(top: 20),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Text(value.toStringAsFixed(0), style: TextStyle(fontSize: 44, fontWeight: FontWeight.w700, color: color, height: 1.0)),
          const SizedBox(height: 2),
          const Text('ATTENTION', style: TextStyle(fontSize: 10, letterSpacing: 2, color: AppColors.textMuted, fontWeight: FontWeight.w600)),
        ]),
      )),
    ));
  }
...
class _PoseTile extends StatelessWidget {
  const _PoseTile({required this.label, required this.value, required this.nominal});
  final String label, value;
  final bool nominal;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.surface0, borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0x33444748))),
      child: Column(children: [
        Text(label, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 2, color: AppColors.textSecondary)),
        const SizedBox(height: 8),
        Text(value, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
        const SizedBox(height: 4),
        Text(nominal ? 'NOMINAL' : 'ALERT', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: color)),
      ]),
    );
  }
}
...
class _ActionButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Material(color: AppColors.surface2, borderRadius: BorderRadius.circular(8),
...
class _MjpegViewState extends State<_MjpegView> {
  ...
  void _onFrame() {
    ...
    if (now.difference(_lastDecodeTime) < _minDecodeInterval && _frame != null) {
  ...
  void _decode(Uint8List bytes) {
    ...
    ui.instantiateImageCodec(bytes).then((codec) {
      if (!mounted || !_decoding) { codec.dispose(); return; }
      codec.getNextFrame().then((info) {
        if (!mounted || !_decoding) { info.image.dispose(); codec.dispose(); return; }
        codec.dispose();
  ...
  Widget build(BuildContext context) {
    final frame = _frame;
    if (frame == null) {
      return ColoredBox(color: AppColors.surface0, child: const _CameraLoadingPlaceholder());
    }
    return RawImage(image: frame, fit: BoxFit.cover, filterQuality: FilterQuality.medium);
  }
}
    ...
    return Stack(
      ...
      children: [
        ...
        Column(mainAxisSize: MainAxisSize.min, children: [
          Container(
            width: 48, height: 48,
            decoration: BoxDecoration(color: AppColors.surface2, shape: BoxShape.circle, border: Border.all(color: const Color(0x33444748))),
            child: const Icon(Icons.videocam_outlined, size: 24, color: AppColors.textMuted)),
          const SizedBox(height: 12),
